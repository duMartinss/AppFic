...
const UserListScreen = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await axios.get('http://4.228.217.151:3000/api/usuario'); // Rota que retorna a lista de usuários
  ...
  const toggleUserStatus = async (userId, currentStatus) => {
    ...
    console.log('Dados da requisição:', {
      ...
      newStatus,
      url: `http://4.228.217.151:3000/api/usuario/status/${userId}`
    });

    try {
      const response = await axios.put(`http://4.228.217.151:3000/api/usuario/status/${userId}`, {
        status: newStatus
      }, {
        headers: {
